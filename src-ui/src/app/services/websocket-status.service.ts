...
connect() {
    ...
    // ۱. پروتکل ws یا wss را بر اساس پروتکل صفحه (http یا https) تعیین کن
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    
    // ۲. هاست فعلی را بگیر (مثلا: localhost:4200 یا آدرس کلودفلر)
    const host = window.location.host;

    // ۳. این آدرس باید با چیزی که در proxy.conf.json تنظیم کردید مطابقت داشته باشد
    // (فرض می‌کنیم آدرس WebSocket شما روی /ws/ تنظیم شده)
    const wsPath = '/ws/status/'; // اگر آدرس دیگری است، اینجا اصلاح کنید

    const wsUrl = `${protocol}//${host}${wsPath}`;

    // از URL ساخته شده جدید استفاده کن
