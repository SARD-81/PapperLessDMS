...
export class DashboardComponent extends ComponentWithPermissions {
  ...
  get subtitle() {
    if (this.settingsService.displayName) {
      return $localize`سلام ${this.settingsService.displayName},به استوریکس خوش آمدی`
    } else {
      return $localize`به استوریکس خوش آمدی`
    }
  }
  ...
  onDrop(event: CdkDragDrop<SavedView[]>) {
    ...
    this.settingsService
      ...
      .subscribe({
        next: () => {
          this.toastService.showInfo($localize`داشبورد بروزرسانی شد.`)
        },
        error: (e) => {
          this.toastService.showError($localize`در بروزرسانی مشکلی به وجود آمده است.`, e)
        },
      })
  }
}
